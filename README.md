...
**v1.2の改善点:**
- ✅ 互換性改善版（ArrayList不使用で幅広いExcelバージョンに対応）
...
## ⚠️ 重要: 使用するファイル
**必ず `KurumawariMacro_Simple.bas` (v1.2) を使用してください。**
`KurumawariMacro.bas` (v1.1) は互換性問題があるため非推奨です。
...
2. VBAエディタ上部メニューの「挿入」をクリックして、モジュールを追加
   - 表示される項目: 「Module」「モジュール」「標準モジュール」のいずれか(Excelバージョンにより異なる)
3. **`KurumawariMacro_Simple.bas` (v1.2)** の内容を**全て**コピー＆ペースト
   - ⚠️ 注意: **1行目の `Attribute VB_Name = "KurumawariMacro"` は除く**（ある場合）
